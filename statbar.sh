...
eval "$(echo "$input" | jq -r '
    "model=" + (.model.displayName // .model.display_name // .model.id // "?" | @sh),
    "cwd=" + (.cwd // "" | @sh),
    "total_cost=" + (.cost.total_cost_usd // .cost.totalCostUsd // .cost.total_cost // .cost.total // 0 | @sh),
    "session_cost=" + (.cost.session_cost_usd // .cost.sessionCostUsd // .cost.session_cost // .cost.session // .cost.total_cost_usd // 0 | @sh),
    "transcript_path=" + (.transcript_path // .transcriptPath // "" | @sh),
    "max_context=" + (.context_window.context_window_size // .context_window.contextWindowSize // 200000 | @sh),
    "pct_fallback=" + (.context_window.used_percentage // .context_window.usedPercentage // 0 | @sh)
...
if [[ -n "$display_msg" ]]; then
    ...
    if [[ "$current_state" == "user" ]]; then
        ...
        if [[ ${#display_msg} -gt $avail_len ]]; then
            echo -e "${line2_prefix}${display_msg:0:$((avail_len - 3))}..."
        else
            echo -e "${line2_prefix}${display_msg}"
        fi
        
    elif [[ "$current_state" == "tool" ]]; then
        echo -e "🛠️  ${C_ACCENT}Running Tool:${C_RESET} ${display_msg}"
    elif [[ "$current_state" == "tool_result" ]]; then
        echo -e "⚙️  ${C_GRAY}${display_msg}${C_RESET}"
    elif [[ "$current_state" == "assistant" ]]; then
        echo -e "⏳ ${C_ACCENT}${display_msg}${C_RESET}"
    elif [[ "$current_state" == "error" ]]; then
        echo -e "⚠️  ${C_RED}${display_msg}${C_RESET}"
    fi
fi
